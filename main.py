...
total_lines = total_lines_in_file(source_file_path)

#sim_thread = Thread(target=simulate_logging, args=(source_file_path, target_file_path))
#sim_thread.start()

# Read log files
log_lines = read_static_log_file(source_file_path)
log_lines_dynamic = follow_log_file(source_file_path)
...
def run(log_lines, total_lines, ip_location_cache, ip_datas, bad_lines):
    for line_number,line in enumerate(tqdm(log_lines, total=total_lines, desc="Processing log lines"), start=1):
        ...
        save_single_ip_data('/home/aysenur/projects/nginx_analyzer/ip_datas_source_static.json', parsed_line.get('ip'), ip_data)
